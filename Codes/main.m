...
dataFolder = fullfile('E:\BUET_CDI\31\312\project\Dataset_Final_V2.0');
commands = categorical(["Bangladesh","Prokoushol", "Bisshobiddaloy","BUET","Cafeteria","Hall","Sher-e-Bangla","Rashid","Nazrul"]);   %"bame","dane","pichone","shamne","thamo"
...
[afe1, afe_params] = DeclareAFE();   %only afe2
segmentSamples = afe_params(1);
frameSamples = afe_params(2);
hopSamples = afe_params(3);
overlapSamples = afe_params(4);
numBands1 = afe_params(5);
% numBands2 = afe_params(5);
numBands=numBands1;
%numBands=numBnads2+numBands1;
...
%features1 = extract(afe1,xPadded);   %will be edited//////////////////////////
features1 = extract(afe1,xPadded);
%stackedFeatures = cat(2, features1, features2);
[numHops,numFeatures] = size(features1);



%% Generate XTrain by feature extraction
params = [reduceDataset, numHops, numBands, segmentSamples];
XTrain = CreateXTrain(adsTrain, afe1, params);
...
XValidation = CreateXValidation(adsValidation,afe1,params);
size(XValidation);
...
Xbkg = CreateXBackground(adsBkg,afe1, params);
